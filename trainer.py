...
class Trainer:
    ...
    def __init__(self, args, ngpus_per_node):
        ...
        self.args.use_similar = False
        build_tokenizer(args)
        args.add_distance = False
        # create model
        logger.info("=> creating model")
        logger.info('Add distance loss: {}, use similar negative samples: {}'.format(self.args.add_distance, self.args.use_similar))

        self.model = build_model(self.args)
        # ckt_dict = torch.load('./checkpoint/wn18rr/checkpoint_epoch18.mdl', map_location=lambda storage, loc: storage)
    ...
    def eval_epoch(self, epoch) -> Dict:
        ...
        for i, batch_dict in enumerate(self.valid_loader):
            ...
            outputs = get_model_obj(self.model).compute_logits(output_dict=outputs, batch_dict=batch_dict, mode='eval')
            outputs = ModelOutput(**outputs)


            logits, labels = outputs.logits, outputs.labels
            # loss = self.criterion(logits, labels)
            loss = get_model_obj(self.model).cal_rince(logits)
    ...
    def train_epoch(self, epoch):
        # logger.info('Whether Embeddings in model has been initialized: {}'.format('yes' if self.model.initial else 'no'))
        losses = AverageMeter('Loss', ':.4')
        top1 = AverageMeter('Acc@1', ':6.2f')
        top3 = AverageMeter('Acc@3', ':6.2f')
        ...
        for i, batch_dict in enumerate(self.train_loader):
            ...
            if self.args.use_amp:
                ...
                outputs = self.model(**batch_dict)
            outputs = get_model_obj(self.model).compute_logits(output_dict=outputs, batch_dict=batch_dict, mode='train')
            outputs = ModelOutput(**outputs)
            logits, labels = outputs.logits, outputs.labels
            ...
            loss = get_model_obj(self.model).cal_rince(logits)
            loss += get_model_obj(self.model).cal_rince(logits[:, : logits.size(0)].t())
            if self.args.add_distance:
                loss += 0.1 * outputs.distance_loss
            acc1, acc3 = accuracy(logits, labels, topk=(1, 3))
            top1.update(acc1.item(), batch_size)
            top3.update(acc3.item(), batch_size)
        ...
        if self.model.initial == False:
            self.model.initial = True
