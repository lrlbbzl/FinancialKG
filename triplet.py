...
class EntityDict:
    ...
    def __init__(self, entity_dict_dir: str, inductive_test_path: str = None):
        ...
        fp = open(path, 'r', encoding='utf-8')
        self.entity_exs = [EntityExample(**obj) for obj in json.load(fp)]
        fp.close()
...
class LinkGraph:
    ...
    def get_nhop_entity_with_degree(self, entity_id: str,
                                 entity_dict: EntityDict,
                                 n_hop: int = 2,
                                 # return empty if exceeds this number
                                 max_nodes: int = 100000) -> dict:
        if n_hop < 0:
            return set()
        seen_eids = dict()
        queue = deque([entity_id])
        for i in range(n_hop):
            ...
            for _ in range(len_q):
                ...
                for node in self.graph.get(tp, set()):
                    if node not in seen_eids:
                        queue.append(node)
                        seen_eids.update({node : i + 1})
                        if len(seen_eids) > max_nodes:
                            return dict()
        res = dict()
        for e_id, degree in seen_eids.items():
            if degree not in res:
                res.update({ degree : set() })
            res[degree].add(entity_dict.entity_to_idx(e_id))
        return res # 1,2,3...
