...
class CustomBertModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        if self.args.use_similar:
            self.entity_embedding = torch.zeros([self.entity_num, 768]).to('cuda')
        self.initial = False
    ...
    def forward(self, hr_token_ids, hr_mask, hr_token_type_ids,
        ...
        hr_vector, h_in_hr, r_in_hr = self._encode(self.hr_bert,
        ...
        # update model's entity embedding
        if self.args.use_similar:
            batch_data = kwargs['batch_data']
            head_id = torch.LongTensor([entity_dict.entity_to_idx(ex.head_id) for ex in batch_data]).to(head_vector.device)
            tail_id = torch.LongTensor([entity_dict.entity_to_idx(ex.tail_id) for ex in batch_data]).to(head_vector.device)
            self.entity_embedding[head_id] = head_vector
            self.entity_embedding[tail_id] = tail_vector
    ...
    def compute_logits(self, output_dict: dict, batch_dict: dict, mode: str) -> dict:
        ...
        # add similarity-base negative samples
        if self.args.use_similar and self.training and self.initial:
            similar_logits = self._compute_similar_batch_logits(hr_vector, tail_vector, batch_dict)
            logits = torch.cat([logits, similar_logits], dim=-1)
        ...
        if self.args.add_distance and mode == 'train':
            distance_loss = self.create_neg_examples_loss(output_dict['h_in_hr'], output_dict['r_in_hr'],
                                        tail_vector, 3)
        else:
            distance_loss = torch.tensor([])
